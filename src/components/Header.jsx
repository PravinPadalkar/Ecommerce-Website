  ...
  return (
    <header className=" px-4 md:px-8 py-4 h-[72px] bg-white flex justify-between shadow-md rounded-sm items-center mb-2 ">
      <div className="flex items-center gap-4 cursor-pointer " onClick={() => navigate("/")}>
        ...
        <div className="hidden md:block text-2xl">My Ecommerce</div>
      </div>
      <div className="flex gap-4 md:gap-12">
        <div className="flex gap-4 items-center mt-1">
          <Badge
            color="success"
            content={cartList.reduce((acc, { quantity }) => acc + quantity, 0)}
          >
            <FontAwesomeIcon
              icon={faCartShopping}
          ...
          <Badge color="success" content={wishList.reduce((acc) => acc + 1, 0)} variant="shadow">
            <FontAwesomeIcon
              icon={faHeart}
              className="fa-2xl text-red-800 cursor-pointer"
